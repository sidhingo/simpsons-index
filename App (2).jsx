...
function ModalImage({ p }) {
  const [errored, setErrored] = useState(false);

  if (p.image_url && !errored) {
    return (
      <div style={{ width:"100%", height:220, background:"#f0ede6", overflow:"hidden" }}>
        <img
          src={p.image_url}
          alt={p.episode_title}
  ...
  const match = p.episode_code?.match(/S(\d+)E(\d+)/);
  const season = match ? match[1] : "?";
  const ep = match ? match[2] : "?";
  return (
    <div style={{
      width:"100%", height:220, background:"#f5f3ee",
      display:"flex", flexDirection:"column",
      alignItems:"center", justifyContent:"center",
      borderBottom:"1px solid #e0ddd6", gap:8,
    }}>
      <div style={{
