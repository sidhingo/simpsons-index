...
// Frinkiac covers S1-S17 only. For S18+ we show a styled placeholder.
// We search Frinkiac with a short quote from the prediction text.
function useFrinkiacImage(episodeCode, predictionText) {
  const [img, setImg] = useState(null);
  const [tried, setTried] = useState(false);

  useEffect(() => {
    if (!episodeCode || !predictionText) { setTried(true); return; }

    // Parse season number
    const match = episodeCode.match(/S(\d+)E/);
    const season = match ? parseInt(match[1]) : 99;

    // Frinkiac only has S01-S17
    if (season > 17 || season === 0) { setTried(true); return; }

    // Build a short search query from the prediction text (first 4-5 words)
    const words = predictionText.replace(/[^a-zA-Z0-9 ]/g, "").split(" ").slice(0, 5).join(" ");

    fetch(`https://frinkiac.com/api/search?q=${encodeURIComponent(words)}`)
      .then(r => r.json())
      .then(data => {
        if (data && data.length > 0) {
          // Find a result from the matching episode if possible, else take first
          const match = data.find(d => d.Episode === episodeCode) || data[0];
          setImg(`https://frinkiac.com/img/${match.Episode}/${match.Timestamp}.jpg`);
        }
        setTried(true);
      })
      .catch(() => setTried(true));
  }, [episodeCode, predictionText]);
...
const BADGE_STYLES = {
  true:     { bg:"#d4edda", color:"#1a5c1a", border:"#6aaa6a", label:"Confirmed"  },
  false:    { bg:"#f8d7da", color:"#B22222", border:"#c07070", label:"Wrong"      },
  pending:  { bg:"#fff3cd", color:"#6b4b00", border:"#c8a43a", label:"Pending"    },
...
// Styled placeholder for S18+ or when no image found
function ImagePlaceholder({ episodeCode, episodeTitle }) {
  const match = episodeCode?.match(/S(\d+)E(\d+)/);
  const season = match ? match[1] : "?";
  const ep = match ? match[2] : "?";
  return (
    <div style={{
      ...
      <div style={{ textAlign:"center" }}>
        <div style={{ fontSize:11, fontFamily:"'Courier New',monospace", color:"#aaa", letterSpacing:1 }}>
          Season {season} · Ep {ep}
        </div>
        {episodeTitle && (
          <div style={{ fontSize:11, color:"#bbb", fontFamily:"Georgia,serif", fontStyle:"italic", marginTop:4, maxWidth:280, textAlign:"center" }}>
            {episodeTitle}
          </div>
        )}
      </div>
...
function ModalImage({ episodeCode, predictionText, episodeTitle }) {
  const { img, tried } = useFrinkiacImage(episodeCode, predictionText);

  if (!tried) {
    return (
      <div style={{ width:"100%", height:220, background:"#f5f3ee", display:"flex", alignItems:"center", justifyContent:"center", borderBottom:"1px solid #e0ddd6" }}>
        <span style={{ fontSize:11, letterSpacing:2, textTransform:"uppercase", color:"#ccc", fontFamily:"Georgia,serif" }}>Loading…</span>
      </div>
    );
  }
    ...
    return (
      <div style={{ width:"100%", height:220, overflow:"hidden", background:"#f0ede6" }}>
        <img
          src={img}
          alt={episodeTitle}
          style={{ width:"100%", height:"100%", objectFit:"contain", objectPosition:"center", background:"#f0ede6" }}
          onError={() => {}}
        />
      </div>
    );
  ...
  return <ImagePlaceholder episodeCode={episodeCode} episodeTitle={episodeTitle} />;
}
    ...
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ padding:"20px 22px" }}>
          ...
          <a
            href={`https://frinkiac.com/?q=${encodeURIComponent(p.prediction_text.slice(0,40))}`}
            target="_blank" rel="noopener noreferrer"
            style={{ fontSize:9, letterSpacing:2, textTransform:"uppercase", color:GOLD, textDecoration:"none" }}
          >
            Search this scene on Frinkiac →
          </a>
        </div>
      </div>
    </div>
  ...
  return (
    ...
    >
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", borderTop:"1px solid #eee", paddingTop:10 }}>
        <span style={{ fontSize:13, fontFamily:"'Courier New',monospace", color:"#888", letterSpacing:0.5 }}>
          {p.episode_code} · {p.air_date ? p.air_date.slice(0,4) : "—"}
        </span>
...
const TAB_DESC = {
  ...
  browse:     `All ${PREDICTIONS.length} predictions including debunked claims`,
};
  ...
  return (
    <div style={{ fontFamily:"Georgia,serif", background:"#fff", color:"#111", minHeight:"100vh" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", borderBottom:"2px solid #111" }}>
        {[
          { key:"true",     num:counts.true,     label:"Confirmed true", col:"#1a5c1a" },
          { key:"pending",  num:counts.pending,  label:"Pending",         col:GOLD      },
          { key:"false",    num:counts.false,    label:"Wrong",           col:"#B22222" },
